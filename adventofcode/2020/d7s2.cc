...
int CountBagsHelper(const map<string, Bag>& bags, string color) {
    cout << "Counting bags for " << color << endl;

    int count = 1;
    const Bag& bag = bags.at(color);
    for (int i = 0; i < bag.contains_colors.size(); ++i) {
        int counted_bags = CountBagsHelper(bags, bag.contains_colors[i]);
        int bag_count = bag.contains_counts[i];
        cout << "  Counted bags for " << color << " is " << counted_bags << endl;
        cout << "  Bag count for " << color << " is " << bag_count << endl;
        count += (counted_bags * bag_count);
    }

    cout << "Returning " << count << " bags for " << color << endl;
    return count;
}

int CountBags(const map<string, Bag>& bags, string color) {
    return CountBagsHelper(bags, color) - 1;  // Minus one for the top-level bag
}
...
int main(int argc, char* argv[]) {
    ...
    cout << "Total bags contained in shiny gold: " << count << endl;
    return 0;
}
